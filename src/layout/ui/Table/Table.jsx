...
export const Table = ({ data, page, refetch }) => {
    const startId = (page - 1) * DEFAULT_PAGE_SIZE + 1;

    const filteredData = data

    const mutation = useMutation(MutationFn({ query: SHOP_DELETE_MUTATION() }));

    // Update Shop
    const navigate = useNavigate();

    const handleUpdateShop = (id) => {
        navigate(`/update-shop?id=${id}`);
    };

    // Delete Shop
    const deleteShop = (id) => {
        mutation.mutate({ shopDeleteId: id }, {
            onSuccess: ({data}) => {
                toast.error(data?.shopDelete.shop_name + " deleted with SKU: " + data?.shopDelete.shop_sku);
                refetch();
            },
            onSettled: () => {
        ...
        <div className="overflow-x-auto relative shadow-md w-full rounded">
            {data && Array.isArray(data) ? (
                <table className="w-full text-sm text-left text-white overflow-y-auto">
                    ...
                    <tr>
                        <th scope="col" className="py-3 px-6 text-white">#</th>
                        {Object.keys(data[0])
                    ...
                    {filteredData && filteredData.map((row, index) => (
                        <tr key={index} className="bg-white border-b hover:bg-white">
                            ...
                            {Object.values(row)
                                .slice(1,3)
                                .map((cell, i) => (
                                    <td key={i} className="py-4 px-6 text-gray-900 whitespace-nowrap">
                                        {cell}
                                    </td>
                                ))}
